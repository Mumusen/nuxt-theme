...
const actions = {
    ...
    getTestCodeRegister({commit, state}, obj){
        axios({
            method: 'get',
            url: Util.url() + 'member/mobileSendMessageService',
            responseType: 'json',
            params: obj
        }).then((res) => {
            if(res){
        ...
        // if(STORAGE){
            axios({
                ...
                if(res){
                    ...
                    commit(types.OPTIONAL_STOCK_LIST, {login: true, ...res.data, param: obj.infoType});
                }
            });
        // }else{
