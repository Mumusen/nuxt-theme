...
<div class="themeDetails" v-if="themeConceptInfo && themeConceptInfo.code">
    <h2>{{themeConceptInfo.conceptStockInfoVo.conceptStockName}} 
        <nuxt-link v-if="currentTheme && currentTheme.id" :to="'/themeDetails?id='+currentTheme.id" style="font-size:14px;" target="_blank">查看更多</nuxt-link> 
    </h2>
    <p class="details">
...
export default {
    ...
    computed: {
        ...mapGetters(['themeConceptInfo', 'currentTheme', 'currentInfo']),
        // nowDate(){
        //     let date = null;
        //     setInterval(()=> {
    ...
    methods: {
        ...mapActions(['saveCurrentInfo', 'getRelationThemeQuo', 'getInfoRelationTheme']),
