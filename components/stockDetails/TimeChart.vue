...
export default {
  ...
  methods: {
    chartInit(newPrice) {
      ...
      // 最大涨幅
      let maxChg = this.valueChgArr.sort((a, b) => {
        return b - a;
      })[0];
      // 最大跌幅
      let minChg = this.valueChgArr.sort((a, b) => {
        return a - b;
      })[0];
      // 最小价格
      ...
      // 最大成交数
      let maxVolume = this.value_volume.sort((a, b) => {
        return b - a;
      })[0];
      // console.log(maxChg);
      // console.log(minChg);
      ...
      let interval = this.intervalPrice(minChg, maxChg, newPrice);
      ...
      let option = {
        ...
        yAxis: [
          // 图一
          {
            ...
            min: function(params) {
              if (Math.abs(minChg) <= Math.abs(maxChg)) {
                // 如果最大涨幅大于0  取最大涨幅的相反数 为 最小涨幅
                return (
                  newPrice +
                  newPrice * (-Math.abs(maxChg) / 100)
                ).toFixed(2);
              } else {
                return (
                  newPrice +
                  newPrice * (-Math.abs(minChg) / 100)
                ).toFixed(2);
              }
            },
            // min: minPrice,
            ...
            max: function(params) {
              if (Math.abs(minChg) <= Math.abs(maxChg)) {
                // 如果最大涨幅大于0  取最大涨幅的相反数 为 最小涨幅
                return (newPrice + newPrice * (Math.abs(maxChg) / 100)).toFixed(2);
              } else {
    ...
    handleData(data) {
      ...
      if (data && data.length !== 0) {
        this.ymd = `${data[0].mintime.slice(0, 4)}-${data[0].mintime.slice(
          4,
          6
        )}-${data[0].mintime.slice(6, 8)}`;
        data.map((item, index) => {
          // console.log(item.mintime)
          let y_m_d_h_m_s = `${item.mintime.slice(0, 4)}-${item.mintime.slice(
            4,
            6
          )}-${item.mintime.slice(6, 8)} ${item.mintime.slice(
            8,
            10
          )}:${item.mintime.slice(10, 12)}`;

          // 时间
          let time = `${item.mintime.slice(8, 10)}:${item.mintime.slice(
            10,
            12
          )}`;
          timeArr.push(time);
          ...
          // 记录下一次的成交量
          this.nextVolume = data[index + 1]
            ? data[index + 1].businessAmount
            : item.businessAmount;
          ...
          if (index > 0 && index < data.length - 1) {
            value_volume.push(
              Math.floor((this.nextVolume - this.prevVolume) / 100)
            );
            volumeArr.push(
              // [
            ...
            value_volume.push(Math.floor(item.businessAmount / 100));
            volumeArr.push(
              // [
              // new Date(y_m_d_h_m_s),
              Math.floor(item.businessAmount / 100)
              // ]
            );
          }
        });
    ...
    // 刻度距离
    intervalPrice(min, max, price) {
      console.log(min, max, price);
      let interval = "";

      if (Math.abs(max) > Math.abs(min)) {
        interval = (2 * price * (Math.abs(max) / 100) / 5).toFixed(2);
      } else {
        interval = (2 * price * (Math.abs(min) / 100) / 5).toFixed(2);
      }
