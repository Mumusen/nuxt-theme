<template>
    <div class="sole-ref-container">
        <div class="sole-ref-list-container">
            <div class="sole-ref-list" v-if="internalReference && internalReference.dataList">
                <div @click="changeNews(info.infomation.id)" class="ref-list-item" v-for="info in internalReference.dataList">
            ...
            <!--   -->
            <div class="page-container" v-if="internalReference && internalReference.count >= 10">
                <page-select 
                    @changePage="recentChangePage" 
                    :totalNum="internalReference.count" 
                    :pageSize="10"
                ></page-select>
            </div>
        </div>
        ...
        <div class="sole-ref-detail" v-if="referenceDetails">
            ...
            <div class="rela-stock">
                ...
                <h4 class="stock" v-for="stock in referenceDetails.details.stockVoList">
                    <nuxt-link :to="'/stockDetails?name='+wordEncodeURI(stock.infoName) + '&code=' + stock.infoCode" target="_blank">
                        {{stock.infoName}}
                    </nuxt-link>
                </h4>
...
export default {
    ...
    computed: {
        ...mapGetters(['internalReference', 'referenceDetails']),
    },

    watch: {
        'internalReference'(newVal, olVal){
            // console.log(newVal);
    ...
    mounted(){
        if(this.internalReference){
           this.getReferenceDetails({
                id: this.internalReference.dataList[0].infomation.id
            }) 
        }
    },

    methods: {
        ...mapActions(['getInternalReference', 'getReferenceDetails']),
        // 翻页
